...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        Iy2 = Iy * Iy;
        IxIy = Ix * Iy;
        GIx2 = scipy.ndimage.gaussian_filter(Ix2, 0.5, mode='reflect')
        GIy2 = scipy.ndimage.gaussian_filter(Iy2, 0.5, mode='reflect')
        GIxIy = scipy.ndimage.gaussian_filter(IxIy, 0.5, mode='reflect')
        trace = GIx2 + GIy2;
        harrisImage = GIx2 * GIy2 - GIxIy * GIxIy - 0.1 * trace * trace;
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        # TODO-BLOCK-BEGIN
        # destImage= np.logical_and(harrisImage == scipy.ndimage.filters.maximum_filter(harrisImage,size=7), harrisImage>0.001)
        destImage = (harrisImage == scipy.ndimage.filters.maximum_filter(harrisImage, size=7))
        # raise Exception("TODO in features.py not implemented")
        # TODO-BLOCK-END
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        sp = grayImage.shape
        newshape = (sp[0] + 4, sp[1] + 4)
        padGray = np.zeros(newshape, np.float32)
        padGray[2:-2, 2:-2] = grayImage
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        zerof = np.zeros(windowSize * windowSize, np.float32)
        ...
        for i, f in enumerate(keypoints):
            ...
            ang = f.angle
            MT1 = get_transmx(-x, -y)
            MR = get_rotmx(-ang * np.pi / 180)
            MS = get_scalemx(0.2, 0.2)
            MT2 = get_transmx(4, 4)
            TT = np.dot(MT2, np.dot(MS, np.dot(MR, MT1)))
            transMx = TT[:2, :]
            # raise Exception("TODO in features.py not implemented")
            # TODO-BLOCK-END
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        minidx = np.argmin(dist, axis=1)
        for i, idx in enumerate(minidx):
            # print 'i=',i,', idx=',idx,'\n'
            mt = cv2.DMatch(i, idx, dist[i, idx])
            matches.append(mt)
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i, idx in enumerate(minidx):
            minimum = dist[i, idx]
            maximum = np.amax(dist[i])
            dist[i, idx] = maximum
            secmin = np.amin(dist[i])
            mt = cv2.DMatch(i, idx, minimum / secmin)
            matches.append(mt)
        # print 'i=',i,', idx=',idx,'distance=',minimum/secmin,'\n'
        # raise Exception("TODO in features.py not implemented")
        # TODO-BLOCK-END
