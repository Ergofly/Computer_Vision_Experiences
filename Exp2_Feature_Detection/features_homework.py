...
import Exp2_Feature_Detection.transformations
...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        # TODO-BLOCK-BEGIN
        Ixp=ndimage.sobel(srcImage,axis=1,mode="reflect")
        Iyp=ndimage.sobel(srcImage,axis=0,mode="reflect")
        IxpIyp_Gus=ndimage.gaussian_filter(np.multiply(Ixp,Iyp),sigma=0.5,mode="reflect")
        Ixp2_Gus=ndimage.gaussian_filter(np.multiply(Ixp,Ixp),sigma=0.5,mode="reflect")
        Iyp2_Gus=ndimage.gaussian_filter(np.multiply(Iyp,Iyp),sigma=0.5,mode="reflect")
        harrisImage=np.multiply(Ixp2_Gus,Iyp2_Gus)-np.multiply(IxpIyp_Gus,IxpIyp_Gus)-0.1*(Ixp2_Gus+Iyp2_Gus)*(Ixp2_Gus+Iyp2_Gus)
        orientationImage = np.arctan2(Iyp, Ixp) * 180 / np.pi
        # TODO-BLOCK-END
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        # TODO-BLOCK-BEGIN
        maximaImage=ndimage.maximum_filter(harrisImage,size=(7,7))
        destImage=(harrisImage==maximaImage)
        # TODO-BLOCK-END
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        # 补零
        grayWithEdge=np.pad(grayImage,[[2,2],[2,2]],"constant",constant_values=np.float32(0.0))
        ...
        for i, f in enumerate(keypoints):
            ...
            # TODO-BLOCK-BEGIN
            desc[i]=grayWithEdge[y:y+5,x:x+5].reshape(1,25)[0]
            # TODO-BLOCK-END
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            trans2=[[1,0,4],
                    [0,1,4],
                    [0,0,1]]
            transMx=np.dot(trans2,np.dot(scale,np.dot(rot,trans1)))[:2][:]
            # TODO-BLOCK-END
            ...
            if np.std(destImage)<1e-5:
                desc[i]=np.zeros(windowSize * windowSize, np.float32)
            else:
                destImage = (destImage - np.mean(destImage)) / np.std(destImage)
                desc[i] = destImage.reshape(1,windowSize*windowSize)[0]
            # TODO-BLOCK-END
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i , mi in enumerate(minIndex):
            match=cv2.DMatch(i,mi,distance[i][mi])
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        minIndex = np.argmin(distance, axis=1)
        for i, mi in enumerate(minIndex):
            disMin=distance[i][mi]
            temp=np.delete(distance[i],mi)
            secMinIndex=np.argmin(temp)
            disSec=temp[secMinIndex]
            matches.append(cv2.DMatch(i, mi, disMin/disSec))
        # TODO-BLOCK-END
