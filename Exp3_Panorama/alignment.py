...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    # TODO-BLOCK-BEGIN
    sum=np.sum(Vt[-1])
    H = (np.array(Vt[-1])/sum).reshape(3, 3)
    # TODO-BLOCK-END
    # END TODO
    return H
...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        if m == eTranslate:
            ran = random.randint(0, num_matches-1)
            ma = matches[ran]
            (a_x, a_y) = f1[ma.queryIdx].pt
            (b_x, b_y) = f2[ma.trainIdx].pt
            M = np.array([[1, 0, b_x - a_x],
                          [0, 1, b_y - a_y],
                          [0, 0, 1]])
        elif m == eHomography:
            mlist = []
            ...
            while mlength < 4:
                ran = random.randint(0, num_matches-1)
                if matches[ran] not in mlist:
                    mlist.append(matches[ran])
                    mlength+=1
            M = computeHomography(f1, f2, mlist)
        else:
            raise Exception("Error: Invalid motion model.")
        inlier_indices = getInliers(f1, f2, matches, M, RANSACthresh)
        if len(inlier_indices) > len(max_inlier_set):
            max_inlier_set = inlier_indices
    M = leastSquaresFit(f1, f2, matches, m, max_inlier_set)
    # TODO-BLOCK-END
    # END TODO
    return M
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        # TODO-BLOCK-BEGIN
        m = matches[i]
        firp = np.array(f1[m.queryIdx].pt)
        secp = np.array(f2[m.trainIdx].pt)
        firp_M = np.dot(M,np.array([firp[0], firp[1], 1]).T)
        cmp = np.array([firp_M[0], firp_M[1]] / firp_M[2])# 齐次化
        dist = np.linalg.norm(cmp - secp)
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i in range(len(inlier_indices)):
            ...
            # TODO-BLOCK-BEGIN
            ma = matches[i]
            u += f2[ma.trainIdx].pt[0]-f1[ma.queryIdx].pt[0]
            v += f2[ma.trainIdx].pt[1]-f1[ma.queryIdx].pt[1]
            # TODO-BLOCK-END
            # END TODO
