...
def imageBoundingBox(img, M):
    ...
    x = img.shape[0]
    y = img.shape[1]
    cornerpt = np.array([[0, 0, 1],
                    [0, x - 1, 1],
                    [y - 1, 0, 1],
                    [y - 1, x - 1, 1]]).T
    cornerpt_m = np.dot(M, cornerpt)
    cornerpt_m = cornerpt_m / cornerpt_m[-1]
    minX = min(cornerpt_m[0])
    minY = min(cornerpt_m[1])
    maxX = max(cornerpt_m[0])
    maxY = max(cornerpt_m[1])
    #TODO-BLOCK-END
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    imgRGBA = np.ones((img_h, img_w, 4))
    imgRGBA[:, :, :3] = img
    (x_mesh, y_mesh) = np.meshgrid(range(img_w), range(img_h))
    alpha = np.fmin(np.fmin((1 + x_mesh) / (blendWidth + 1), (img_w - x_mesh) / (blendWidth + 1)), 1)
    alpha *= np.sum(img, axis=2) > 0
    for i in range(4):
        imgRGBA[:, :, i] *= alpha
    acc += cv2.warpPerspective(imgRGBA, M, (acc_w, acc_h), cv2.INTER_LINEAR)
    #TODO-BLOCK-END
    # END TODO
